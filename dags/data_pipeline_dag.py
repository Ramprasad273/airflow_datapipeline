...
Task 3: Read from the staging table, transform and insert data
        into the "popular_destination_history" table.
Task 4: Read the latest data inserted into "popular_destination_history"
...
with DAG('New_york_taxi_dag', default_args=default_args, schedule_interval='@monthly',
    ...
    check_if_file_exists = BashOperator(task_id='check_file',
                                        bash_command='shasum ~/data_files_airflow/green_tripdata.csv',
                                        retries=2,
    ...
    prepare_history_table = PythonOperator(task_id='insert_into_history_table',
                                           python_callable=populate_history_data,
                                           provide_context=True)
